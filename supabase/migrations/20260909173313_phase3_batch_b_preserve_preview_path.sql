-- Phase 3 Batch B — fixes a real correctness gap found during
-- implementation: upsert_order's whole-child-set-replace deletes and
-- reinserts print_specs on every save without carrying preview_storage_path
-- forward (the RPC's insert list never included it, by design — previews
-- are written via a separate plain UPDATE after generation, since Storage
-- operations can't participate in this RPC's transaction). That meant
-- EVERY save nulled preview_storage_path first, then relied entirely on
-- the client's post-save preview-sync step to set it back — so if preview
-- generation failed after a successful save, the previously-good preview
-- reference was already gone from the DB, even though the underlying
-- Storage object was untouched and still valid. This violates the
-- required failure boundary ("keep existing preview_storage_path where
-- appropriate" when preview regeneration fails).
--
-- Fix: snapshot existing (id, preview_storage_path) pairs before the
-- delete, and restore them onto any reinserted row with a matching id
-- (ids are stable across saves per Milestone 1's identity fix). The
-- client's preview-sync step still overwrites this immediately after with
-- a freshly generated preview when it succeeds — this only changes what
-- happens when it fails.
--
-- Verified live via MCP: created a print spec, set a preview path,
-- resaved with different position/size/rotation/offsets/approval note —
-- preview_storage_path survived unchanged while every other field updated
-- correctly and the print spec's id stayed stable.
...
begin
  ...
  delete from _prev_print_spec_previews;
